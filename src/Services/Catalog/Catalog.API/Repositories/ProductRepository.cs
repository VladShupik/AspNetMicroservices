...
namespace Catalog.API.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<IEnumerable<Product>> GetProductByName(string name) => await _context.Products.Find(p => p.Name == name).ToListAsync();

        public async Task<IEnumerable<Product>> GetProductByCategory(string categoryName)
            => await _context.Products.Find(p => p.Category == categoryName).ToListAsync();
